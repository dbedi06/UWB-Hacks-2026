...
export function mapReportRowToClient(row) {
    ...
    return {
        ...
        // Prefer the cluster-coalesced image_url (set by the GET aggregation
        // window function) so a cluster keeps showing a photo even when the
        // most-recent member didn't attach one. Falls back to the row's own
        // image_url for direct-from-POST responses (no aggregation).
        image_url:
            typeof row.cluster_image_url === "string" ? row.cluster_image_url
